...
uint8_t spi_transfer(uint8_t data) {
    uint8_t received = 0;
    for (int i = 7; i >= 0; i--) {
        gpioWrite(MOSI_PIN, (data >> i) & 0x01);
        gpioWrite(SCLK_PIN, 1);
        received <<= 1;
        if (gpioRead(MISO_PIN)) received |= 1;
        gpioWrite(SCLK_PIN, 0);
    }
    return received;
}
...
int sd_init(void) {
    ...
    gpioSetMode(SCLK_PIN, PI_OUTPUT);
    gpioSetMode(CS_PIN, PI_OUTPUT);

    gpioWrite(CS_PIN, 1);
    gpioWrite(MOSI_PIN, 1);
    gpioWrite(SCLK_PIN, 0);
